...
seed_count_closed_MIX <- seed_count_closed_MIX %>%
  rename("mix" = Transect)
seed_count_open_MIX <- seed_count_open_MIX %>%
  rename("mix" = Transect)

# join open seeds data and closed seed data
final_df1 <- right_join(av_nestedness_final, seed_count_open_MIX, by = "mix")
final_df2 <- right_join(final_df1, seed_count_closed_MIX, by = "mix")
