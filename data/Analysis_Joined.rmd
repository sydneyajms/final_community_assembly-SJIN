...
veg_data <- veg %>%
  ...
  subset(Stand %in% c("604","605","607","611","700","702","704","705")) %>%
  subset(Site == 6) %>%
...
ThisMix <- Stand_df[[i]] %>%
  group_by(Pollinator, PlantGenusSpecies.y) %>%
  summarize(interactions = n(), .groups = 'drop') %>%
  pivot_wider(names_from = Pollinator, values_from = interactions) %>%
  column_to_rownames(var = "PlantGenusSpecies.y") 
  WHOLE_LIST[[i]] <- ThisMix
}

WHOLE_LIST1 <- lapply(WHOLE_LIST, function(df) {
  mat <- as.matrix(df)
  rownames(mat) <- rownames(df)  # preserve plant species names
  return(mat)
})

WHOLE_LIST2 <- lapply(WHOLE_LIST1, function(df) {
  # preserve names
  rn <- rownames(df)
  cn <- colnames(df)
  
  # convert to numeric matrix safely
  mat <- matrix(as.numeric(unlist(df)), nrow = nrow(df), ncol = ncol(df))
...
nestedness_values <- sapply(WHOLE_LIST2, function(mat) networklevel(mat, index = "weighted NODF"))
