...
# paste stand and mix and split seed data by these combos (option 1)
...
# try averaging by mix instead (option 2)
...
Join null and actual nestedness grouped by stand and mix combo 

```{r}
# separtate vector into unique stand and mix and create data frame then (RUN FOR BOTH)
df_nested <- tibble(
  name = nested_values_st) %>%
  separate(name, into = c("stand", "mix", "date", "nestedness"), sep = "_", remove = TRUE)
  
# join null to actual (RUN FOR BOTH)
df_all <- df_nested %>%
...
```
Join null and actual nestedness grouped by mix 

```{r}
# find average for every stand and mix combo by averaging nestedness of different dates
nestedness_av_final_byMIX <- df_all %>%
  group_by(mix) %>%
  mutate(nestedness = as.numeric(nestedness)) %>%
  summarise(
    av_nestedness_actual = mean(nestedness, na.rm = TRUE), 
...
#nestedness_av_final$Standsr1 <- paste(nestedness_av_final$stand, nestedness_av_final$mix, sep = "_")
nestedness_av_final_byMIX$Stand <- NULL
nestedness_av_final_byMIX$Standsr1 <- NULL

nestedness_av_final <- nestedness_av_final %>%
  filter(Standsr1 != "605_Mix2") # this row was ommitted in the seed data and thus is extra

write.csv(nestedness_av_final_byMIX, "nestedness_av_final_byMIX.csv", row.names = FALSE)
```
```
