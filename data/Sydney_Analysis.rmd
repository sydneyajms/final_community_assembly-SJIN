...
# 
# Stand_df_mat <- vector("list", length = length(Stand_df))
# for(i in seq_along(Stand_df)){
# this.mat <- as.matrix(Stand_df[[i]])
#   rownames(this.mat) <- rownames(Stand_df[[i]])
#   Stand_df_mat[[i]] <- this.mat
# }
...
for(i in seq_along(WHOLE_LIST)){
  ...
  nodf <- networklevel(this.mat, index = "connectance")
